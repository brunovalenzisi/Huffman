...
public class Console {
	...
	private static final TriFunction<Character, Integer, String, Character> HEX = (c, kc,
			s) -> isHexDigit(c) || _validKeyCode(kc) ? c : null;
	private static final TriFunction<Character, Integer, String, Character> CHAR = (c, kc,
			s) -> s.length() < 3 || _validKeyCode(kc) ? c : null;
	private static final TriFunction<Character, Integer, String, Character> INTEGER = (c, kc,
			s) -> Character.isDigit(c) || _validKeyCode(kc) ? c : null;
	private static final TriFunction<Character, Integer, String, Character> DOUBLE = (c, kc,
			s) -> s.isEmpty() && c == '-' || Character.isDigit(c) || c == '.' && s.indexOf('.') < 0 || _validKeyCode(kc)
					? c
					: null;
	private static final TriFunction<Character, Integer, String, Character> BOOLEAN = (c, kc,
			s) -> parseBoolean(s) != null || _validKeyCode(kc) ? c : null;
	private static final TriFunction<Character, Integer, String, Character> UPPERCASE = (c, kc, s) -> Character
			.toUpperCase(c);
	private static final TriFunction<Character, Integer, String, Character> LOWERCASE = (c, kc, s) -> Character
			.toLowerCase(c);
	private static final TriFunction<Character, Integer, String, Character> FLAG = (c, kc,
			s) -> c == '1' || c == '0' && s.length() < 2 || _validKeyCode(kc) ? c : null;
	private static final TriFunction<Character, Integer, String, Character> AÑZ = (c, kc,
			s) -> c >= 'A' && c <= 'Z' || c == 'Ñ' || _validKeyCode(kc) ? c : null;
	private static final TriFunction<Character, Integer, String, Character> AZ = (c, kc,
			s) -> c >= 'A' && c <= 'Z' || _validKeyCode(kc) ? c : null;
	...
	public String _readString(TriFunction<Character, Integer, String, Character> mask) // ,BiFunction<Character,String,Character>
																						// mask)
	{
		try {
			reading = true;
	...
	public class EscuchaAnyKey extends KeyAdapter {
		...
		public void keyPressed(KeyEvent e) {
			...
			if (anyKey == KeyEvent.VK_SHIFT || anyKey == KeyEvent.VK_CONTROL || anyKey == KeyEvent.VK_ALT
					|| anyKey == KeyEvent.VK_ALT_GRAPH /* || anyKey==KeyEvent.VK_ENTER */) {
				return;
			}
	...
	private void _finalizarElPrograma() {
		int r = JOptionPane.showConfirmDialog(frame, "¿Esta acción finalizará el programa?", "Confirmación",
